...
** signal
*** kill -> process
**** task_struct -> signal_struct -> shared_pending shared by all threads in the process
*** tgkill -> thread
...
**** get_unused_fd_flags
***** fdtable->open_fds->find_next_fd->find_next_zero_bit->__ffs->__buildin_ctzl(count trailing zero bits long)
