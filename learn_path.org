...
**** semop
** socket
*** socket(int domain, int type, int protocol)创建一个file, struct socket, 从数组inetsw里根据type protocol查找struct inet_protosw
**** socket->ops = answer->ops(aka. inet_stream_ops)
**** 创建struct sock *sk, sk->sk_prot = answer->tcp_prot, struct socket对接用户，struct sock对接网络协议栈, tcp_prot是协议栈中的函数
**** struct sock维护3个队列以及状态
...
****** add wait queue entry to target file wake queue, wait queue entry has a `func` which is called when file has events
******* #+begin_src c
			...
			wait_queue_entry_t *bookmark)
{
	...
	return nr_exclusive;
				...
				#+end_src
****** wait queue entry func is `ep_poll_callback`
****** wait entry and epitem is associated together with struct eppoll_entry
******* #+begin_src c
struct eppoll_entry {
...
****** poll_table knows how to add wait queue entry to target file wakeup lists queue, but dont know the queue,
****** so it becomes the callback handled to vfs_poll, file provides the wait queue, file_operations invoke the callback with its wait queue
***** ep_item_poll
****** vfs_poll
******* file->fop->poll(file, pt)
******* for socket, it call socket_file_ops->poll -> sock_poll -> sock->ops->poll -> tcp_prot->poll -> tcp_poll -> sock_poll_wait
******* -> poll_wait(file, &sock->wq.wait, p) -> p->qproc
******* socket file wait queue is &sock->wq.wait
******* invoke the poll callback when events ready
******** for socket, tcp_data_queue -> tcp_data_ready -> sk->sk_data_ready -> sock_def_readable -> wake_up_interruptible_sync_poll
...
* https
** 结合非对称加密和对称加密，以兼顾效率和安全性
** (网站域名 + 公钥)由CA机构私钥加密得到证书
** 客户端用CA公钥解密网站签名，若解密成功并且网站域名和请求网站域名相同，签名安全，取出网站公钥
** 利用公钥沟通对称加密的密钥和加密套件
** 对称加密传输数据
