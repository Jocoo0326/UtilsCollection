...
******* initialized in af_net.c inet_init => proto_register(&tcp_prot, 1) -> sock_register(&inet_family_ops){.create = inet_create} -> inet_register_protosw
...
** edge-triggered
*** only notify when changes occurred on monitored file descriptor
